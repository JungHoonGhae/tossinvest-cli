...
| `tossctl accumulate funding-status [--full]` | Securities accumulation funding, automated-order registration, and trade-purpose MyData state (`banking status` is a deprecated alias; not general Banking/MyData; holder/account masked, internal ID never emitted) |
