...
| `tossctl banking status [--full]` | 증권 주식모으기 자금연결·자동주문 등록·거래목적 확인용 MyData 상태(일반 Banking/MyData 아님, 예금주·계좌번호 기본 마스킹, 내부 ID 항상 미출력) |
| `tossctl notifications list` | WTS 알림 설정 조회(읽기 전용, 내부 사용자 ID 제외) |
| `tossctl notifications status` | 받은함 미확인·AI 이슈 SNS 공개·FOMC 라이브·AI 분석 콘텐츠 알림 및 동의 상태 조회(읽기 전용) |
