...
conversation. tossctl's API surface is **104 operations** (and growing) — exposed as individual
...
Result: always-on context = **exactly 3 tool schemas**. Whether there are 40 operations or 104, the
...
- **Reads** — the official API (accounts, quotes, order book, trades, candles, …) plus **WTS-only**
  ...
  `trading_settings`, `securities_transfer_accounts`, and `banking_status` respectively. The
