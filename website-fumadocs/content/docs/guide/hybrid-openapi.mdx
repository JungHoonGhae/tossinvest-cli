...
증권사에게 앱은 단순한 거래 창구가 아니라 핵심 자산입니다. 사용자가 앱 안에 오래 머물수록
콘텐츠·커뮤니티·추천·광고 같은 부가 가치를 함께 붙일 수 있고, 다른 상품과의 연계 판매도
가능해집니다. 그래서 새 기능은 **언제나 앱(WTS)에 가장 먼저** 실리고, 공식 Open API는 그중
외부에 안정적으로 노출해도 되는 일부만 선별해 **뒤따라** 공개합니다 — 비즈니스 구조상 의도적으로
보수적일 수밖에 없습니다.

그래서 수급·시장지수·AI 시그널·스크리너·실시간 푸시처럼 "토스다운" 기능일수록 WTS에 먼저,
더 빨리 도착합니다. tossctl이 **WTS를 1차 경로로** 두는 이유가 여기에 있습니다 — 공식 API의
로드맵을 기다리지 않고 지금 쓰기 위해서. 동시에 공식이 지원하는 기능은 공식 경로로 자동
라우팅해 **안정성도 놓치지 않습니다.** 더 빠른 범위와 더 안정적인 경로, 둘 다 취하는 셈입니다.
...
<Mermaid chart={`flowchart TD
  A[요청] --> B{공식 API가 지원하는 기능?}
  B -- 아니오 --> W[WTS 웹 세션]
  B -- 예 --> C{공식 키 등록됨?}
  C -- 아니오 --> W
  C -- 예 --> D[공식 Open API · OAuth]
  D -- 호출 성공 --> R[결과]
  W --> R
  D -. 일시적 실패 .-> E{주문인가?}
  E -- 예 --> X[오류 · 중복주문 방지]
