...
### Why the official API always lags

For a brokerage, the app isn't just an order window — it's the core asset. The longer users stay
inside it, the more it can layer on content, community, recommendations, and ads, and cross-sell
other products. So new features land **in the app (WTS) first**, and the official Open API
exposes only the slice that's safe to open externally, **afterwards** — structurally conservative
by design, not by accident.

That's why the most "Toss-native" features — investor flows, indices, AI signals, screener,
real-time push — reach WTS first and fastest. That's exactly why tossctl treats **WTS as the
primary path**: to use them now instead of waiting on the official roadmap. At the same time, it
auto-routes officially-supported features through the official path, so you **don't give up
stability** either — you get the faster surface and the steadier route at once.
...
<Mermaid chart={`flowchart TD
  A[Request] --> B{Officially supported?}
  B -- No --> W[WTS web session]
  B -- Yes --> C{Official key present?}
  C -- No --> W
  C -- Yes --> D[Official Open API · OAuth]
  D -- Success --> R[Result]
  W --> R
  D -. temporary failure .-> E{Order?}
  E -- Yes --> X[Error · prevent duplicate order]
  E -- No --> W
`} />
