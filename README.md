...
> 공식 한도는 빡빡합니다 — 계좌·보유 조회는 연속 호출의 80% 이상이 429 ([실측](docs/migration/open-api.md#rate-limit-실측)). tossctl 은 429 를 감지해 자동으로 WTS 로 폴백하므로 반복 조회·모니터링에서도 끊김이 없습니다.
...
- **안정성(복원력)** — 공식 Open API 는 계좌·보유 조회에 빡빡한 rate limit 을 겁니다 (실측: 연속 호출 시 **계좌 80%·보유 87% 가 429**). tossctl 은 기본 자동 라우팅이 429 를 감지해 웹 세션으로 즉시 폴백 → 체감 **100% 성공, 중단 없음**. 공식 한도가 곧 tossctl 의 한도가 되지 않습니다. [실측·방법론 ↗](docs/migration/open-api.md#rate-limit-실측)
