...
- **속도** — 증권사는 사용자가 앱에 머물며 콘텐츠·추천·광고까지 소비하길 원하므로, 새 기능은 늘 웹앱(WTS)에 먼저 실리고 공식 API 는 그중 일부만 뒤따라 엽니다(구조적으로 보수적). tossctl 은 주간 모니터로 신규 엔드포인트를 잡아 **공식 출시를 기다리지 않고 먼저 구현**합니다. [왜 공식 API가 늦는지 ↗](https://tossinvest-cli.vercel.app/docs/guide/hybrid-openapi)
