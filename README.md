...
> 공식 한도는 빡빡합니다 — 계좌·보유 조회는 연속 호출의 80% 이상이 429 ([실측 3개 시점](docs/migration/open-api.md#rate-limit-실측), 수치는 토스 정책에 따라 변동 가능). tossctl 은 429 를 감지해 자동으로 WTS 로 폴백하므로 반복 조회·모니터링에서도 끊김이 없습니다.
...
- **안정성(복원력)** — 공식 Open API 는 계좌·보유 조회에 빡빡한 rate limit 을 겁니다 (실측 3개 시점: 연속 호출의 **80%+ 가 429**, 계좌 82%·보유 84%). tossctl 은 기본 자동 라우팅이 429 를 감지해 웹 세션으로 즉시 폴백 → 체감 **100% 성공, 중단 없음**. 한도 수치는 토스 정책에 따라 언제든 바뀔 수 있지만, 공식 한도가 곧 tossctl 의 한도가 되지 않는 구조적 이점은 동일합니다. [실측·방법론 ↗](docs/migration/open-api.md#rate-limit-실측)
