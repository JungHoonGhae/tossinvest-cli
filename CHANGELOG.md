...
### 새 기능
- **`market calendar`** — 다가오는 경제 일정(미국 지표 발표 등)을 날짜별로 봅니다. 토스의 AI 한 줄 요약이 함께 나옵니다.

  ```
  미국 고용지표와 주요 기업 실적 발표가 이어져요

  2026-08-05
    종일   비농업부문 고용변화량(ADP) 발표
    종일   ISM 서비스업 구매관리자지수 발표
  ```

  - 서버가 기간 파라미터를 받지 않아 **항상 앞으로 약 10일치**입니다. 지난 일정은 제공되지 않습니다.
  - 시각이 `종일`인 항목은 서버가 발표 시각을 주지 않는 경우입니다(내부적으로 `23:59:59.999999999` 로 옵니다).
  - MCP·`tossctl ops call economic_calendar` 로도 노출됩니다. 웹앱(WTS) 전용.

### 개선
- **미확인 API 후보를 한 번에 훑는 개발 도구가 생겼습니다** (`tools/probe_candidates.py`) — 기여자용입니다. WTS 카탈로그의 `candidate` 는 주간 모니터가 **신규·삭제만** 알려줘서, 처음 쌓인 날 눈에 안 띈 경로는 계속 방치됐습니다(2026-08-03 기준 358개 중 346개가 45일째 미확인). 이 도구가 GET 으로 한 번씩 찔러 `worth-review` / `needs-params` / `thin` / `forbidden` 등으로 갈라 카탈로그에 기록하므로, 다음에 무엇을 볼지 정할 수 있습니다. 응답 본문은 저장하지 않습니다.
