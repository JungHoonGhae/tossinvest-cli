...
### 새 기능
- **`account trading-settings`** — 간편주문 활성 상태, KRX/NXT 체결시장 선택, ATS 알림, 옵션 실시간 시세 요청·서비스·과금 관련 원문 플래그를 한 번에 조회합니다. 모두 증권 WTS의 읽기 계약이며 설정을 변경하지 않습니다.
- **`account transfer-accounts [--full]`** — 증권 주식이체 화면에서 제공하는 내 계좌와 최근 목적지 계좌를 조회합니다. 계좌번호는 CLI·ops/MCP에서 기본 마스킹하고, 이 명령은 이체를 시작하거나 계좌 상태를 바꾸지 않습니다.
...
- API 회귀 감시를 65개로 확대해 목표가 알림·숨긴 보유종목, 국가별 브리핑, AI 시그널 상세, 업종 상세 5개 의존 API, 어닝콜 상세, 대여수익 랭킹에 더해 오픈뱅킹 상태 2개·증권 거래 설정 4개·주식이체 계좌 2개의 읽기 계약을 감시합니다. 여러 API를 합치는 operation은 `ExtraProbes`로 모든 의존성을 등록합니다. 주간 모니터는 WTS build ID 변경과 일반 Toss Android 배포 후보도 함께 알리며, APK 후보는 서명·정적 감사를 마치기 전까지 `stale` 신호로만 취급합니다. Android 결과에는 `live`/`offline` 메타데이터 출처도 표시해 테스트 fixture가 실제 신규 배포처럼 보이지 않게 했습니다.
