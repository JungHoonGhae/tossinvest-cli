...
- API 회귀 감시를 76개로 확대해 목표가 알림·숨긴 보유종목, 국가별 브리핑, AI 시그널 상세, 업종 상세 5개 의존 API, 어닝콜 상세, 대여수익 랭킹, 오픈뱅킹·거래목적 확인 상태 5개·증권 거래 설정 4개·주식이체 계좌 2개·계좌 접속 및 제한 상태 3개에 더해 전 계좌 합산/단일 계좌 자산 추이·이력·일자 상세 6개의 읽기 계약을 감시합니다. 여러 API를 합치는 operation은 `ExtraProbes`로 모든 의존성을 등록하고, 공용 `account-list` 의존성은 `ProbeRefs`로 중복 호출 없이 추적합니다. 계좌 범위 API probe에는 런타임에 확인한 기본 `accountKey`를 넣고, 커서 종료 페이지의 `nextCursorKey: null`도 정상 계약으로 처리합니다. 주간 모니터는 WTS build ID 변경과 일반 Toss Android 배포 후보도 함께 알리며, APK 후보는 서명·정적 감사를 마치기 전까지 `stale` 신호로만 취급합니다. Android 결과에는 `live`/`offline` 메타데이터 출처도 표시해 테스트 fixture가 실제 신규 배포처럼 보이지 않게 했습니다.
...
- 현재 WTS 번들의 정적 경로·호출 근거와 읽기 전용 라이브 응답을 교차 검증한 19개 계약을 endpoint 카탈로그에 반영했습니다. 화면 노출 여부가 아니라 현재 세션으로 실제 호출 가능한 host·method·응답 타입을 구현 근거로 사용합니다.
