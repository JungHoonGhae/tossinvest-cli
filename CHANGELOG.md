...
### 변경
- **일반 주문과 조건주문의 안전 정책을 `trading.Service` 하나로 통합했습니다.** CLI와 ops/MCP가 같은 config 옵트인, execute, confirm-token 검사를 거치므로 새 호출 표면에서도 게이트가 달라지지 않습니다. OCO/OTO ops 입력은 두 번째 trigger 누락도 공식 API 호출 전에 거절합니다.
