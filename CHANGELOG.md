...
### 새 기능
- **`account trading-settings [--account <key>]`** — 간편주문 활성 상태, KRX/NXT 체결시장 선택, ATS 알림, 옵션 실시간 시세 요청·서비스·과금 관련 서버 원문 플래그를 한 번에 조회합니다. 기본 계좌 외 다른 증권 계좌도 고를 수 있고 결과에는 원래 키 대신 불투명한 account scope를 표시합니다. 모두 증권 WTS의 읽기 계약이며 설정을 변경하지 않습니다.
...
- **`banking status [--full]` 보강** — 기존 주식모으기 자금연결 정보에 새 연결 생성 가능 여부와 등록 필요 여부를 추가했습니다. 일반 Toss Banking이 아니라 계속 `securities + wts` 범위입니다.
