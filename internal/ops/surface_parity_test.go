...
var opsOnlyByDesign = map[string]string{
	"BuyingPower": "공식의 매수여력은 WTS 계좌요약과 개념이 달라 드롭인 대체재가 아니다. " +
		"같은 커맨드에 얹으면 백엔드에 따라 다른 뜻의 숫자가 나온다.",
	"ExchangeRate": "WTS 는 복수 통화쌍을, 공식은 단일 쌍을 준다. 시그니처가 달라 " +
		"`market fx` 에 그대로 얹히지 않는다.",
	...
	"MarketCalendar": "거래일 캘린더. KR/US 스키마가 갈려 표 형태가 정해지지 않았다. 노출 자체는 안전하다.",
