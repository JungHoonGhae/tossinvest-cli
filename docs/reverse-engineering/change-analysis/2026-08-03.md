...
## 7. 6절 실행 결과 (사람 검토 후 처리)

6.4 의 권장 2·3번을 처리했고, **6.2 의 판단 하나가 그새 낡았다.**

| 6절 항목 | 처리 |
|---|---|
| 6.4-3 소수점 접수 마감 안내 | **반영** — `--fractional` 도움말 + README(ko/en) + CHANGELOG (#147) |
| 6.2 "갱신할 사용자 대면 문서 없음" | **더 이상 사실 아님** — 위 반영으로 문서가 생겼다 |
| 6.4-1 별도 코드 변경 불필요 | 유효. `orderableHours`·`invalidTickSize` 는 패스스루라 무영향 |
| 6.4-2 `invalidTickSize` 실계좌 확인 | 미실행 (필수 아님) |

### 놓칠 뻔한 것: `paths` 만 비교하면 이 변경이 안 보인다

#147 을 만들 때 spec 을 `paths` 기준으로만 diff 해서 **`components` 6건을 통째로
놓쳤다.** 6.1 이 그걸 잡아낸 덕에 다음이 드러났다:

**미국 지정가에도 호가 단위 제약이 있다.** 기존 spec 은 호가 단위를 KR 에만 명시했는데
1.2.9 에서 US 가 추가됐다 (`ConditionRequest.orderPrice`, `OrderCreateRequest.price`,
`OrderModifyRequest.price`):

```
US: 소수점 (달러 단위). 호가 단위에 맞아야 합니다
    ($1 미만은 0.0001, $1 이상은 0.01 단위).
```

tossctl 은 `--price` 를 그대로 보내므로 코드 영향은 없지만, README·도움말 어디에도
호가 단위 언급이 없어(grep 0건) `--price 158.015` 같은 값은 **서버 400 으로만** 알 수
있었다. 6.4-3 과 같은 성격의 빈틈이라 함께 문서화했다.

**다음 분석 때 지킬 것**: spec diff 는 `paths` 만 보지 말고 `components` 까지 본다.
스냅샷이 `spec_paths_sha256` 과 `spec_full_sha256` 을 따로 두는 이유가 이것이다 —
경로 수가 그대로여도(27 → 27) 내용은 바뀐다.
