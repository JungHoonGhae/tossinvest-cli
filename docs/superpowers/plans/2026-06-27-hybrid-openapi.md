...
흐름(TTY): 인증 방식 선택(`Select`, **각 옵션에 니즈별 트레이드오프 설명**):
- "공식 Open API 키만 — 핵심·공식 지원 기능만. 가장 안정적(토큰 자동 갱신, 브라우저 불필요, 키 1년)"
- "웹 세션 로그인만 — 고유·비공식 기능(시그널·수급·관심종목·커뮤니티·배당·푸시 등)까지. 단 더 빨리 끊기고 갱신 주기 짧음"
- "둘 다 (권장) — 안정성 + 최대 범위, 공식 장애 시 폴백"
→
