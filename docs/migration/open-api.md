...
> 실측 2026-06-30 · 단일 자격증명 · **3개 독립 시점** · 시점별 엔드포인트당 15~20회 순차 버스트(지연 없음) · stdout 폐기(계좌 PII 미노출), stderr 의 폴백 통지로 official 429 판정.

공식 spec 은 rate limit 수치를 공개하지 않는다. `--backend openapi` 로 엔드포인트를 연속 호출해
official 이 몇 번째에 429 로 떨어지는지 서로 다른 3개 시점에서 측정·집계했다.

| 엔드포인트 | official 성공 | 429 → WTS 폴백 (집계) | 시점별 범위 | 패턴 |
|---|---|---|---|---|
| `quote get` (`/prices`) | 39/40 | ~3% | 0~5% | ~10 req/s 버스트에도 멀쩡 (시세는 넉넉) |
| `account list` (`/accounts`) | 8/45 | **82%** | 80~86% | 보통 2번째 호출부터 429 |
| `portfolio positions` (`/holdings`) | 7/45 | **84%** | 80~87% | 보통 1번째 호출부터 429 |

→ 계좌·보유 계열은 사실상 **연속 1회 후 수 초간 429** — 15회 버스트에서 official 성공은 2~3회뿐.
시세(`/prices`)는 throttle 이 거의 없다. 즉 공식 단독으로는 반복 조회(대시보드·모니터·스크립트·
에이전트 루프)가 대부분 실패하고, 3개 시점 모두 80%+ 로 재현됐다.
...
> **이 수치는 토스의 비공개 rate-limit 정책에 달려 있어 언제든 바뀔 수 있다** (공식 spec 미공개).
> 자격증명·시점·엔드포인트별로도 다르며, 위 값은 2026-06-30 버스트 기준 관측치다. 핵심은 절대 수치가
> 아니라 **"공식 단독은 반복 조회에 취약하고, tossctl 은 폴백으로 이를 가린다"는 구조** — 토스가 한도를
> 바꿔도 이 구조적 이점은 유지된다.
