...
`monitor api` 명령은 56개 read-only endpoint 를 schema-invariant probe 로 호출해 이런 변경을 사용자보다 먼저 감지합니다.
...
런타임 목록인 `internal/monitor.Probes()` 가 단일 진실 소스입니다. 50개는
...
2026-05-13 10:00 UTC — 1/56 probes failed
