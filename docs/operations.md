...
`monitor api` 명령은 76개 read-only endpoint 를 schema-invariant probe 로 호출해 이런 변경을 사용자보다 먼저 감지합니다.
...
런타임 목록인 `internal/monitor.Probes()` 가 단일 진실 소스입니다. 71개는
...
| 개인화·계좌 부가기능 | `community-rankings`, `lending-expected`, `lending-top-revenue`, `accumulation-plans`, `profit-overview`, `ria-report`, `open-banking-status`, `open-banking-creatable`, `open-banking-registration`, `auto-trading-open-banking`, `trade-purpose-mydata-account`, `notification-settings`, `price-alerts`, `watchlist`, `watchlist-groups` (15) |
...
2026-05-13 10:00 UTC — 1/76 probes failed
