...
- **Repository Metadata**: 매일 08:17 KST에 통계·스타 차트·후원자를 한 PR로 갱신합니다.
  다운로드는 매일 관측하며, 미병합 PR의 이전 관측도 보존합니다. 별·포크 과거 값은 현재
  남아 있는 목록으로 역산한 값이라 취소·삭제 시 과거 행도 바뀔 수 있습니다.
- **Daily Monitor**: 하루 두 번 API 생존과 공식 스펙을 확인합니다. 실제 스펙·지원 범위
  마커 변경만 PR로 남깁니다. **WTS API Monitor**의 주간 웹 빌드·APK 후보 감시는 유지합니다.
- 각 자동화는 생성 전에 같은 종류의 열린 bot PR을 재사용하고 최신 main을 정상 병합합니다.
  force-push는 하지 않으며, 충돌·중복 PR·예상 밖 파일·외부 PR은 중단하여 사람이 검토합니다.
- main이 바뀌면 **Refresh Automation PRs**가 허용된 데이터 PR만 최신화합니다.
  GITHUB_TOKEN 자동 병합은 후속 push workflow를 만들지 않으므로 6시간마다 복구 점검도 합니다.
  열린 대상이 없으면 브랜치 체크아웃·생성·CI 실행 없이 종료합니다.
  실제 pull_request CI의 실행 승인을 처리하며, 최신 main 기준 필수 검사를 통과해야 병합됩니다.
  CI 생략 커밋이나 별도 dispatch 검사로 우회하지 않습니다.
- Dependabot은 주간 minor/patch 묶음과 major 묶음을 분리하고 생태계별 열린 일반 업데이트를
  최대 2개로 제한합니다. 메이저는 마이그레이션 검토 대상이며 무조건 자동 병합하지 않습니다.
  보안 알림·보안 업데이트는 계속 활성화합니다. Node 타입 major는 문서 서버의 Node 24와 맞춥니다.

실패하면 Actions 로그와 해당 PR의 최신 SHA를 확인하세요. 충돌을 강제로 덮거나 PR을
반복 생성하지 말고 기존 PR을 수정하거나, 관측값을 대체 PR에 보존한 뒤 정리합니다.
