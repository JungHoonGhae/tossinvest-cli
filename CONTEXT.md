...
**주문 결과 확인 (order reconciliation)** — 제출한 주문·정정·취소 요청의 반영 상태를
미체결·완료 내역에서 확인하는 절차로, 주문 재제출이나 사용자 승인이 아니다. 미체결에서
사라진 것만으로 취소 성공을 뜻하지 않으며, 확인되지 않은 결과는 `unknown`으로 구분한다.
_피할 말_: retry (조회 반복과 주문 재제출을 구분하지 못함)
